		        ...
		        <view class="item-title" style="padding: 30rpx 20rpx;background-color: white;margin-bottom: 20rpx;" >
							<view style="display: flex;align-items: center;border-bottom: 1px solid #b4b4b4;justify-content: space-between;padding-bottom: 10rpx;">
								...
								<view >
									...
									<text style="margin-left: 10rpx;background: #00aaff;color: white;padding: 5rpx 10rpx;border-radius: 15rpx;">{{item.recharge_type}}</text>
	...
	export default{
		...
		methods:{
			...
			dialogInputConfirm(val){
				...
				this.$api.refuseRecharge(data).then(res=>{
			...
			pass(id){
				...
				uni.showModal({
					...
					success: (res) => {
						if(res.confirm) {  
							that.$api.passRecharge(data).then(res=>{
			...
			queryList(pageNo, pageSize){
				...
				this.$api.getRechargeApplyList(data).then(res=>{
