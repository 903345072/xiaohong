<template>
	<view class="zaiui-home-box" :class="show?'show':''">
		...
		<view class="" >
			...
			<bar-title bgColor='bg-white' @rightTap="barEditTap" :isBack="false">
				<block slot="content">待出票</block>
				<block slot="left" style="display: flex;align-items: center;">
					
					<view @click="refresh()"  class="cuIcon-refresh" style="font-size: 40rpx;"></view>
		          ...
		          <view class="item" v-for="(item,index) in dataList">
		              <view class="item-title" style="padding: 30rpx 20rpx;background-color: white;margin-bottom: 20rpx;" @click="open(item)">
									<view style="display: flex;align-items: flex-start;justify-content: center;flex-direction: column;padding-bottom: 25rpx;line-height: 50rpx;">
										...
										<view style="display: flex;align-items: center;">
											<view style="margin-right: 200rpx;">注数:{{item["num"]}}</view>
										...
										<view style="display: flex;align-items: center;justify-content: space-between;width: 100%;">
											...
											<view>预计奖金:<text style="color: blue;">{{item.expect_max_award}}</text></view>
										</view>
										...
										<view style="font-size: 22rpx; color: #adadad;display: flex;align-items: center;justify-content: space-between;width: 100%;">
											...
											<view v-if="item['state'] == -1" style="display: flex;align-items: center;">
												<view @click.stop="cancel(item['id'],index)" style="background-color: red;padding: 5rpx 30rpx;border-radius: 10rpx;color: white;">撤销</view>
												<view @click.stop="printTick(item['id'],index)" style="background-color: #00aaff;padding: 5rpx 30rpx;margin-left: 10rpx;border-radius: 10rpx;color: white;">出票</view>
											</view>
	...
	export default {
		...
		watch: {
			...
			scrollBottom() {
				...
				if(this.scrollBottom != 0 &&this.index == 0) {
					  this.getList(1)
				}
			},
		},	
		data() {
			return {
				orderdetail:{},dataList:[],emulator_Height: 0,state:-1,
		...
		methods: {
			...
			getList(flag){
				...
				var data = {"type":this.game_type_arr[this.game_index]["en_name"],"pageNo":this.pageNo,"pageSize":this.pageSize}
